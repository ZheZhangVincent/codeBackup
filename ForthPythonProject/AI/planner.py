...
from plangraph import PreprocessingException, PlangraphPreprocessor
...
def parse_cmd_line_args(valid_encodings, default_encoding, valid_solvers, default_solver):
    ...
    parser.add_argument("-e", "--encoding", dest="encoding", 
        choices=valid_encodings.keys(), default=default_encoding,
            metavar="ENCODING", help="The encoding to use "+\
            "[%(choices)s] (default: %(default)s).")
    parser.add_argument("-s", "--solver", dest="solver", 
        choices=valid_solvers.keys(), default=default_solver,
    ...
    if args.query_strategy == "ramp":
        try:
            horizon, horizon_max, horizon_step = map(int, args.horizons.split(":"))
            if horizon < 1:
            ...
            args.horizons = range(horizon, horizon_max + 1, horizon_step)
        ...
        try:
            horizons = map(int, args.horizons.split(":"))
            if min(horizons) < 1:
                raise Exception()
    ...
    print "    Domain file:          ", args.domain_file_name
    print "    Problem file:         ", args.problem_file_name
    print "    Experiment name:      ", args.exp_name
    if args.query_strategy == "ramp":
        print "    Query strategy:        ramp, min_h =", horizon, "max_h =", horizon_max,\
            "step_h =",horizon_step
    elif args.query_strategy == "fixed":
        print "    Query strategy:        enum, h =", ", ".join(map(str, args.horizons))
    else: assert False
    print "    Compute plangraph:    ", args.plangraph
    print "    Plangraph constraints:", args.plangraph_constraints
    print "    Encoding:             ", args.encoding
    print "    Solver                ", args.solver
    print "    SAT solver time out   ", args.time_out
    print "    Write debug CNF:      ", args.debug_cnf
    print "    Remove tmp files:     ", args.remove_tmp
...
def main():
    ...
    print "Command line arg processing time:", (arg_processing_time - start_time)
    ...
    try:
        ...
        ground_file_name = os.path.join(tmp_path, args.exp_name + GROUND_SUFFIX)
    ...
    try:
        for horizon in args.horizons:
            ...
            try:
                ...
                encoding_wrapper.instantiate_encoding(args.encoding, strips_problem)
                encoding = encoding_wrapper.encoding
                encoding.encode(horizon, args.exec_semantics, args.plangraph_constraints)
                end_encoding_base_time = time.time()
                print "Encoding generation time:", (end_encoding_base_time - step_start_time)
                ...
                print "Writing time:", (end_writing_cnf_time - end_encoding_base_time)
            except Exception as e:
            ...
            if sln_res:
                ...
                if sim_res:
                    ...
                    raise PlanningException("INVALID PLAN!", solving_error_code)
