...
################################################################################
#        Your Encodings need to override the following three methods           #
################################################################################
...
################################################################################
    ...
    def write_annotation_file(self, file_name):
        ...
        """
        try:
            with file(file_name, 'w') as anno_file:
    ...
    def print_true_variables(self, v_type):
        ...
        for cnf_code in self.true_vars:
            ...
            print cnf_code, self.cnf_code_name[cnf_code], "step", self.cnf_code_step[cnf_code] 
...
class EncodingWrapper(object):
    ...
    def instantiate_encoding(self, encoding_name, problem):
        ...
        self.encoding = getattr(encoding_module, encoding_module.encoding_class)(problem)
