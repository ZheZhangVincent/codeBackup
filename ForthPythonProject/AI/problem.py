    ...
    def substitute_derived_predicates(self, derived_predicates, seen_preds):
        ...
        if self.pred in derived_predicates:
            ...
            for dp in derived_predicates[self.pred]:
                ...
                dp_copies.append(dp.precondition.substitute_vars(dict(zip(dp.param_names, self.variables))))
            if len(dp_copies) > 1:
...
class ForAllCondition(Condition):
    ...
    def substitute_vars(self, substitutions):
        ...
        if self.var in substitutions.values():
            for v in lower_var_alphabet:
                if v not in substitutions.values():
                    subs[self.var] = v
                break
    ...
    def make_flat_effects(self, ground_effects, grounding):
        ...
        """
        for cgrounding in self.ground_conditions[grounding].itervalues():
...
class Action(object):
    ...
    def __init__(self, name, parameters, types, precondition, effect, is_derived):
        ...
        self.parameters = zip(parameters, types)
...
class Problem(object):
    ...
    def simplify(self):
        ...
        for pred in neg_precs.itervalues():
    ...
    def make_cond_and_cond_eff_lists(self):
        """ Make seperate lists of conditions and conditional effects that will
            ...
            (Problem) -> None
        """
        for cond in self.code_conds.itervalues():
    ...
    def compute_conflict_mutex(self):
        ...
        for action1 in self.actions.itervalues():
    ...
    def make_strips_problem(self):
        ...
        #Propositions
        propositions = {}
        for predicate in self.predicates.itervalues():
        ...
        for (predicate, grounding), prop in neg_propositions.iteritems():
            #Effects
        ...
        if self.first_layer_actions is not None:
            ...
            for action, step in self.first_layer_actions.iteritems():
                if action in actions:
                    problem.action_first_step[actions[action]] = step
         
        #State mutexes   
        if self.state_mutexes is not None:
            problem.state_mutexes = {}
            for step, mutexes in self.state_mutexes.iteritems():
                problem.state_mutexes[step] = []
                for (prop1, grounding1, sign1), (prop2, grounding2, sign2) in mutexes:
                    if not sign1 or not sign2: continue
                    problem.state_mutexes[step].append((propositions[(prop1, grounding1)],
