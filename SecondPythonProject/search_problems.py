    ...
    def get_successors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if next_x >= 0 and next_x < self.width and next_y >= 0 and next_y < self.height and\
                ...
                if player == 0:
                    succ.append(((1, next_pos, black_pos, next_yellow_birds, next_score, max(1, yb_score-1)), action, 1))
                else:
                    succ.append(((0, red_pos, next_pos, next_yellow_birds, next_score, max(1, yb_score-1)), action, 1))
        if not succ:
            succ.append(((1-player, red_pos, black_pos, yellow_birds, score, max(1, yb_score-1)), Directions.STOP, 1))
        return succ
