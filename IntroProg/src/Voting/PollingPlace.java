...
public class PollingPlace {
	...
	public static void main (String args[]) {
		PollingPlace.integrityTest();
	}
	
	public PollingPlace(String string1, int int1, Map pre_map, Map pas_map, Map gap_map, Map temp_map) {
		this.PPNm = string1;
		this.CountNum = int1;
	...
	private static List<PollingPlace> GetData() {
		...
		for (int index = 0; index < (datafromfile.size() - 1); index++) {
			if (((datafromfile.get(index)).get(5)).equals((datafromfile.get(index + 1)).get(5))) {
				...
				PollingPlace pp = new PollingPlace(((String) (datafromfile.get(index)).get(4)), (Integer.parseInt((String) (datafromfile.get(index)).get(5))), pre_map, pas_map, gap_map, temp_map);
				dataLines.add(pp);
				pas_map = new HashMap(pre_map);
				pre_map = new HashMap();
				ter_map = new HashMap();
