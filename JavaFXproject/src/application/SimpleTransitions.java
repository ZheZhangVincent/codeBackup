...
public class SimpleTransitions extends Application {
    ...
    private void init(Stage primaryStage) {
        ...
        rectangle.setOnScroll(new EventHandler<ScrollEvent>() {
            @Override public void handle(ScrollEvent event) {
                ...
                for (Circle c : new Circle[] { circleSmall, circleBig }) {
                    if (c.getTranslateX() + translateX + c.getRadius() > 500) {
                        translateX = 500 - c.getTranslateX() - c.getRadius();
                    }
                    if (c.getTranslateX() + translateX - c.getRadius() < 0) {
                        translateX = - c.getTranslateX() + c.getRadius();
                    }
                    if (c.getTranslateY() + translateY + c.getRadius() > 500) {
                        translateY = 500 - c.getTranslateY() - c.getRadius();
                    }
        ...
        root.getChildren().addAll(rectangle, circleBig, circleSmall);
    }
